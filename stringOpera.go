...
func p() {
	// no matter what kind of input para, just print its value and type of it
	// don't know how to solve it yet
	fmt.Println()
}
func main() {
	fmt.Println("below is tests for string")
	fmt.Println("字符串转化")
	//how to find is it a 64bit or 32bit
	//strconv.IntSize
	...
	if com01 == 0 {
		...
		fmt.Println("no eque " + string(com01))
	}
	fmt.Println(com01)
	//return
	...
	findIt := strings.Contains(str01, "cat")
	fmt.Println(isCon, findIt) //true
	...
	fmt.Println(strings.Index(str01, "haha")) //NO found result: -1
	return
